...
function catchkathmandu_header_meta() {
	...
	*/	
	if ( get_the_author_meta( 'user_url' ) != '' ) {
		$catchkathmandu_author_url = 	esc_url( get_the_author_meta( 'user_url' ) );						  
	}
	else {
		$catchkathmandu_author_url = esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) );
	}	

	$date = sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date updated" datetime="%3$s" pubdate>%4$s</time></a>',
...
if ( ! function_exists( 'catchkathmandu_page_post_meta' ) ) :
/**
 * Post/Page Meta for Google Structure Data
 */
function catchkathmandu_page_post_meta() {
	/* Check Author URL to Support Google Authorship
	* 
	* By deault the author will link to author archieve page
	* But if the author have added their Website in Profile page then it will link to author website
	*/	
	...
	$catchkathmandu_page_post_meta = '<span class="post-time">' . __( 'Posted on', 'catch-kathmandu' ) . ' <time class="entry-date updated" datetime="' . esc_attr( get_the_date( 'c' ) ) . '" pubdate>' . esc_html( get_the_date() ) . '</time></span>';
    $catchkathmandu_page_post_meta .= '<span class="post-author">' . __( 'By', 'catch-kathmandu' ) . ' <span class="author vcard"><a class="url fn n" href="' . $catchkathmandu_author_url . '" title="View all posts by ' . get_the_author() . '" rel="author">' .get_the_author() . '</a></span></span>';

	return $catchkathmandu_page_post_meta;
} 
endif; //catchkathmandu_page_post_meta