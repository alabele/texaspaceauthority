  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        ...
        $this->SelfPath = plugins_url( 'inc/meta-box-class', plugin_basename( dirname( __FILE__ ) ) );
      }else{
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    if (strpos($field_id, '[') === false){
	  ...
	  /* this comment prevent delete permanently the uploaded file
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
		...
		*/
        ...
        echo '<img class="mig-repeated-remove" src="';
        if ($this->_Local_images){
...
.at-inline .at-textarea{width: 100px; height: 75px;}
  ...
  public function show_field_wysiwyg( $field, $meta,$in_repeater = false ) {
    ...
    if ( version_compare( $wp_version, '3.2.1' ) < 1 || $in_repeater )
      ...
      wp_editor( $meta, $field['id'], array( 'editor_class' => 'at-wysiwyg'.( isset($field['class'])? ' ' . $field['class'] : '' )) );
    }
    $this->show_field_end( $field, $meta );
  }
...
/*
  ...
  public function show_field_image( $field, $meta ) {
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      $html .= "<div class='mupload_img_holder'><img src='".$meta['src']."' style='height: auto ;width: 130px;' /></div>";
      ...
      $html .= "<div class='mupload_img_holder'></div>";
