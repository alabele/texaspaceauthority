...
class Advanced_Recent_Posts_Widget extends WP_Widget {
	...
	function __construct() {
    	$widget_ops = array(
			...
			'description' => __('Show recent post with thumbnails and multiple options')
		);
    	parent::__construct('advanced-recent-posts', __('Evident Recent Posts'), $widget_ops);
	}
	...
	function widget($args, $instance) {
			...
			if( ! $thumb_w =  absint($instance["thumb_w"] ))  $thumb_w=80;
		...
		?>
			...
			<li class="recent-post-item clearfix">
            	<div class="image-recent clearfix">
                	<?php
				...
				?>
					...
					<img src="<?php echo get_template_directory_uri().'/'.'inc/thumb.php?src='. $thumbnail[0] .'&amp;h='.$thumb_h.'&amp;w='.$thumb_w.'&amp;z=0'.'&amp;q=100&amp;s=1'; ?>" alt="<?php the_title_attribute(); ?>" width="<?php echo $thumb_w; ?>" height="<?php echo $thumb_h; ?>" />
					
  					</a>
					<?php else:?>
                    <a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
					<img src="<?php echo get_template_directory_uri().'/images/no-image.jpg' ?>" width="<?php echo $thumb_w; ?>" height="<?php echo $thumb_h; ?>" alt="<?php the_title_attribute(); ?>"/>
                    </a>
				<?php endif; ?>
                </div> <!-- End of image recent -->
				<div class="main-recent-wrapper clearfix">
					<div class="main-recent-content">
                	<a class="main-recent-title" href="<?php the_permalink(); ?>" rel="bookmark" title="Permanent link to <?php the_title_attribute(); ?>" ><?php the_title(); ?></a>
					<?php if ( $instance['date'] ) : ?>

					<div class="main-recent-date"><?php the_time("j M"); ?></div>

					<?php endif; ?>
                	</div>
				</div> <!-- End of main recent wrapper -->
	...
	function form( $instance ) {
		...
		$thumb_w = isset($instance['thumb_w']) ? absint($instance['thumb_w']) : 80;
